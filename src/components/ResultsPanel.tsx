    ...
    <div className="space-y-6">
      <div className={`rounded-2xl border-2 p-8 shadow-md ${verdictClasses}`}>
        <div className="flex items-start gap-5">
          <div className="rounded-full bg-white/70 dark:bg-stone-900/60 p-3 shadow-sm shrink-0">
      ...
      <div className="rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 p-6 shadow-sm">
        <h3 className="text-sm font-semibold text-stone-900 dark:text-stone-100 mb-4">Net wealth over time</h3>
    ...
    <div className="rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 p-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          ...
          <p className="text-3xl font-bold tabular-nums text-stone-900 dark:text-stone-100 mt-1">
            {formatGBP(buyTotal)}
            <span className="text-sm font-normal text-stone-500 dark:text-stone-400"> / month</span>
          </p>
      ...
      <p className="mt-4 pt-4 border-t border-stone-200 dark:border-stone-700 text-sm text-stone-700 dark:text-stone-300">
        {diff > 0 ? (
          <>
            Buying costs{' '}
            <span className="font-semibold tabular-nums text-stone-900 dark:text-stone-100">{formatGBP(diff)}/month</span>{' '}
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between">
      ...
      <dd className="tabular-nums text-stone-900 dark:text-stone-100">{value}</dd>
    </div>
  )
}
